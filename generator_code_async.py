...
class LLMInference:
    ...
    def __init__(
        ...
        if self.backend == "vllm":
            self.model_name = self.config.get("model_name", "llama3_3-70b-instruct")
            if api_pool is None:
...
class TreeNode:
    ...
    def format_dim_prompt(self):
        ...
        prompt = f"""As a coding expert, your task is to examine the following python code generation instruction-input pairs to identify the SINGLE most significant dimension that characterizes the instruction-input space and differentiates these instruction-input pairs.
...
"""
    ...
    def format_expand_prompt(self, dimension, attribute_values):
        ...
        prompt = f"""As a coding expert, your task is to supplement the potential attribute values for a specified dimension in order to comprehensively model the entire space of instruction-input pairs.
...
"""
    ...
    def format_gen_prompt(self):
        if self.is_root():
            prompt = """As a coding expert, you are asked to come up with 10 diverse Code Alpaca-style python code generation instruction-input pairs.
...
"""
            ...
            prompt = f"""As a coding expert, you are asked to come up with 10 diverse Code Alpaca-style python code generation instruction-input pairs.
...
11. Input constraints: The input must be a common data type in Python and cannot be a python function.
12. Programming Focus: All instructions must relate to coding or programming.
13.Attributes: Each problem should be associated with all these attributes: {attributes_json}
...
"""
    ...
    async def generate_samples_async(self):
        async def generate_subsamples():
            ...
            all_samples = []
            pattern = r"<Instruction\s+(\d+)>\s*(.*?)(?=\s*<Instruction\s+\d+>|$)"
